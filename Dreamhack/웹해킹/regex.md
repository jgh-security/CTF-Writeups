...
유저 정보를 조회하는 /get_info 페이지를 확인했다. userid를 입력받아 서버 내부 API에 요청을 보내는 구조이다.
...
일반 브라우저에서 ../flag를 입력하면 자동 인코딩 기능 때문에 서버가 경로 이동 명령으로 인식하지 못한다. 이를 우회하기 위해 Burp Suite의 Intercept 기능을 활성화하여 서버로 전송되는 원본 패킷을 가로챘다.

패킷 조작 및 전송

가로챈 패킷의 userid 파라미터 값을 강제로 ../flag로 수정했다. 이렇게 하면 서버 측에서는 http://127.0.0.1:8000/api/user/../flag를 호출하게 되며, 최종적으로 /api/flag에 접근하게 된다.
